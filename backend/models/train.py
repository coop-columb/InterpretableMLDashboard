...
import numpy as np    # For image array representation
...
def load_rareplanes_data(data_dir: Path):
    """
    Loads RarePlanes data. Parses annotations, extracts first image bytes,
    and loads image bytes into NumPy array using Pillow.
    """
    ...
    # --- Start: Image Extraction & Loading (First Image Only) ---
    ...
    try:
        ...
        image_member_name = f"./PS-RGB_tiled/{first_image_id}.png" # Use .png
        logger.info(f"Attempting to extract/load image for ID '{first_image_id}' from member '{image_member_name}'...")
            ...
            try:
                ...
                with tar.extractfile(image_member_info) as f_img:
                    if f_img:
                        ...
                        logger.info(f"  - Successfully extracted {len(image_bytes)} bytes.")
                        ...
                        try:
                             ...
                             logger.info(f"  - Loaded image into numpy array with shape: {image_np.shape} and dtype: {image_np.dtype}")
                             image_loaded = True
                             # TODO: Preprocess image_np (resize, normalize)
                        except Exception as img_err:
                             logger.error(f"  - Failed to load image bytes using Pillow: {img_err}")
                        # --- End Pillow loading ---
                    else: logger.error(f"Could not extract file object for image member: {image_member_name}")
            except KeyError: logger.error(f"Image member '{image_member_name}' not found in archive {image_archive_path}.")
    except Exception as e: logger.exception(f"An unexpected error occurred during image loading: {e}")
    # --- End: Image Extraction & Loading ---

    if not image_loaded: logger.warning("Failed to load the first test image.")

    logger.info("Data loading step finished (annotations processed, first image loaded).")
    return None, None # Still returning None for datasets

# --- start_training function (update status message) ---
def start_training(data_dir: str, epochs: int = 5, batch_size: int = 32, learning_rate: float = 0.001):
    ...
    train_ds, val_ds = load_rareplanes_data(Path(data_dir)) # Calls updated loader
    ...
    return {"status": "Training script finished (placeholder steps, annotations processed, first image loaded)", "model_path": str(model_save_path)}
